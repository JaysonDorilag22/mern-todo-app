...
export default function AddTodoModal({ projectId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(toast, 'Error', error.response?.data?.message || error.message, 'error')
    } finally {
      setLoading(false)
    }
  }
            ...
            <div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button className="w-full text-left">{assignedUser ? assignedUser : 'Select User'}</Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                  <DropdownMenuLabel>Assign User</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => setAssignedUser('')}>None</DropdownMenuItem>
                  {projectDetails && projectDetails.invitedUsers && projectDetails.invitedUsers.map((user) => (
                    <DropdownMenuItem key={user._id} onClick={() => setAssignedUser(user._id)}>
                      {user.name}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
